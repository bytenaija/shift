...
const Homepage = (props) =>{
    ...
    const submitAnswers = async () =>{
        ...
        for(let i =1; i<= 10; i++){
            console.log(typeof response[`question ${i}`] === 'undefined')
            console.log(i);
        ...
        try{
            let {data} = await axios.post('http://localhost:8000/api/answers', response, { headers: {
    ...
    return(
        <div className="homepage">
            ...
            {questions.map(question => <QuestionComponent question={question} handleChange={handleChange} key={question.id} />)} 
           
            <div className="card email">
                <label htmlFor="email">Your Email</label>
                <input className="form-control" name="email" placeholder="you@example.com" onChange={handleChange}/>
            </div>

            <button className='btn btn-primary' onClick={submitAnswers}>Save & Continue</button>
            </div>
        </div>
    )
}
